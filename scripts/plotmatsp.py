...
for file in os.listdir('\\\\sf3\\cicutagroup\\jsm89\\'):
    if ("v1_BF_20x_shutter" in str(file)) and ("forPython" in str(file)):
        ...
        if time<0.001:
            ax1.plot(data1['q'][4:q_max], data1['ps'][4:q_max]*(10**18), label=str(file[18:26])+"s") # q_min: q_max
        else:
            ax2.plot(data1['q'][4:q_max], data1['ps'][4:q_max]*(10**18), label=str(file[18:26])+"s") # q_min: q_max
...
plt.suptitle("Raw PS for different shutter times for vesicle of radius 12um")#+filename[:-14])
plt.xlabel("q (1/m)")
...
plt.title("Cross-section of PS showing amplitude against shutter time for the {}th detected mode".format(cross_sec_q))
